...
class Vector:
    ...
    def norm_1(self):
        f = lambda l: abs(l[0]) if len(l) == 1 else abs(l[0]) + abs(f(l[1:]))
        return f(self.data)
    
    def norm_inf(self):
        f = lambda l: abs(l[0]) if len(l) == 1 else abs(l[0]) if abs(l[0]) > abs(f(l[1:])) else abs(f(l[1:]))
        return f(self.data)
...
class Matrix:
    ...
    def sum(self):
        f = lambda l: Vector([x.sum() for x in l]).sum()
        return f(self.data)

    def max(self):
        f = lambda l: Vector([x.max() for x in l]).max()
        return f(self.data)
